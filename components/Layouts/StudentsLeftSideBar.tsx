...
const StudentsLeftSideBar = () => {
    ...
    return (
        <aside className="fixed z-20  h-15 md:flex flex-col items-center top-0 left-0 bg-[#111] shadow md:p-4 md:h-screen md:w-[17%] w-full">
            <div className="w-full flex justify-between p-4 md:p-0 bg-[#111] h-14">
                <h1 className="text-2xl font-bold mb-6 text-white text-center w-3/4">Bloom</h1>
                {
                    showMenu ?
                        <BiX size={30} className="fill-[white] md:hidden" onClick={() => { setShowMenu(!showMenu) }} />
                        : <BiMenu size={30} className="fill-[white] md:hidden" onClick={() => { setShowMenu(!showMenu) }} />
                }
            </div>

            <nav className={`${showMenu ? 'flex' : 'hidden md:flex'} bg-[#111] font-semibold flex-col md:py-4 space-y-4 w-full text-gray-700`}>
